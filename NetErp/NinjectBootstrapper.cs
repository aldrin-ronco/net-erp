...
namespace NetErp
{
    class NinjectBootstrapper : BootstrapperBase
        ...
        protected override void Configure()
        {
            ...
            _ = kernel.Bind<DianCertificateCache>().ToSelf().InSingletonScope();
            _ = kernel.Bind<IEntityCache>().ToMethod(ctx => ctx.Kernel.Get<DianCertificateCache>());
            ...
            _ = kernel.Bind<EmailCache>().ToSelf().InSingletonScope();
            _ = kernel.Bind<IEntityCache>().ToMethod(ctx => ctx.Kernel.Get<EmailCache>());
            ...
            _ = kernel.Bind<IRepository<MenuItemGroupGraphQLModel>>().To<GraphQLRepository<MenuItemGroupGraphQLModel>>().InSingletonScope();
            ...
            _ = kernel.Bind<IRepository<UserPermissionGraphQLModel>>().To<GraphQLRepository<UserPermissionGraphQLModel>>().InSingletonScope();
