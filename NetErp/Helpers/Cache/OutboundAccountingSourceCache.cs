...
{
    /// <summary>
    /// Cache de fuentes contables de salida de inventario (kardexFlow=O, annulment=false).
    /// Usado por módulos de Stock Movements Out (master grid + new dialog).
    /// </summary>
    public class OutboundAccountingSourceCache : IEntityCache<AccountingSourceGraphQLModel>, IBatchLoadableCache,
        ...
        IHandle<AccountingSourceDeleteMessage>
    {
        ...
        public OutboundAccountingSourceCache(
        ...
        public async Task EnsureLoadedAsync()
        {
            ...
            try
            {
                ...
                object variables = new GraphQLVariables()
                    .For(fragment, "filters", new { KardexFlow = "O", Annulment = false })
                    .Build();

                PageType<AccountingSourceGraphQLModel> result = await _service.GetPageAsync(query, variables);
                ...
                UiDispatcher.Invoke(() =>
                {
                    ...
                    {
                        _items.Clear();
                        foreach (AccountingSourceGraphQLModel item in result.Entries.OrderBy(a => a.Code))
                        {
                            _items.Add(item);
                        }
                        IsInitialized = true;
        ...
        {
            variables.For(batchFragment, "filters", new { KardexFlow = "O", Annulment = false });
        }
        ...
        public void PopulateFromBatchResponse(JToken data)
        {
            ...
            UiDispatcher.Invoke(() =>
            {
                ...
                {
                    _items.Clear();
                    foreach (var item in page.Entries.OrderBy(a => a.Code))
                        _items.Add(item);
                    IsInitialized = true;
                }
            });
        ...
        {
            // Solo agregar si pertenece al filtro de este cache (kardexFlow=O).
            if (item.KardexFlow != 'O') return;
            UiDispatcher.Invoke(() =>
            {
                lock (_lock)
                {
        ...
        public void Update(AccountingSourceGraphQLModel item)
        {
            UiDispatcher.Invoke(() =>
            {
                lock (_lock)
                {
                    ...
                    if (existing != null)
                    {
                        // Si dejó de ser kardexFlow=O, removerlo del cache.
                        if (item.KardexFlow != 'O')
                        {
        ...
        {
            if (message.CreatedAccountingSource?.Entity != null)
            {
