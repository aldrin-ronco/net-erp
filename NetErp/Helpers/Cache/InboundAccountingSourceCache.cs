...
{
    /// <summary>
    /// Cache de fuentes contables de entrada de inventario (kardexFlow=I, annulment=false).
    /// Usado por módulos de Stock Movements In (master grid + new dialog).
    /// </summary>
    public class InboundAccountingSourceCache : IEntityCache<AccountingSourceGraphQLModel>, IBatchLoadableCache,
        ...
        IHandle<AccountingSourceDeleteMessage>
    {
        ...
        public InboundAccountingSourceCache(
        ...
        public async Task EnsureLoadedAsync()
        {
            ...
            try
            {
                ...
                object variables = new GraphQLVariables()
                    .For(fragment, "filters", new { KardexFlow = "I", Annulment = false })
                    .Build();
        ...
        {
            variables.For(batchFragment, "filters", new { KardexFlow = "I", Annulment = false });
        }
        ...
        {
            // Solo agregar si pertenece al filtro de este cache (kardexFlow=I).
            if (item.KardexFlow != 'I') return;
            UiDispatcher.Invoke(() =>
            {
                lock (_lock)
                {
        ...
        public void Update(AccountingSourceGraphQLModel item)
        {
            UiDispatcher.Invoke(() =>
            {
                lock (_lock)
                {
                    ...
                    if (existing != null)
                    {
                        // Si dejó de ser kardexFlow=I, removerlo del cache.
                        if (item.KardexFlow != 'I')
                        {
