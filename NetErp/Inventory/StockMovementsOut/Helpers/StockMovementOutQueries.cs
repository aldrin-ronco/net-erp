...
namespace NetErp.Inventory.StockMovementsOut.Helpers
{
    /// <summary>
    ...
    /// </summary>
    internal static class StockMovementOutQueries
    {
        // -------- Master listing --------
        ...
        // -------- Items con stock disponible (stockTotalsPage) --------
        // Solo expone filas con quantity > 0, agregadas por (item, storage, tracking).
        // Soporta búsqueda parcial multi-token sobre item.name/reference/code via 'matching'.
        public static readonly Lazy<(GraphQLQueryFragment Fragment, string Query)> ItemsWithStockPage = new(() =>
        {
            var fields = FieldSpec<PageType<StockTotalGraphQLModel>>
                ...
                .SelectList(p => p.Entries, e => e
                    ...
                    .Select(s => s.Item, i => i
                        ...
                        .Field(x => x.Reference)
                        .Field(x => x.IsActive)
                        .Field(x => x.IsLotTracked)
                        .Field(x => x.IsSerialTracked)
                        .Field(x => x.AllowFraction)
                        .Select(x => x.MeasurementUnit, mu => mu
                            .Field(m => m.Id)
                            .Field(m => m.Abbreviation))
                        .Select(x => x.SizeCategory, sc => sc
                            .Field(c => c.Id)
                            .Field(c => c.Name)
                            .SelectList(c => c.ItemSizeValues, sv => sv
                                .Field(v => v.Id)
