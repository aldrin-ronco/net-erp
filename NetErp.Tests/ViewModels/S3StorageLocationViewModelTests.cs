...
public class S3StorageLocationViewModelTests
{
    ...
    private readonly S3StorageLocationViewModel _vm;

    public S3StorageLocationViewModelTests()
    {
        _eventAggregator = Substitute.For<IEventAggregator>();
        _service = Substitute.For<IRepository<S3StorageLocationGraphQLModel>>();
        ...
        IRepository<AwsS3ConfigGraphQLModel> awsS3Repo = Substitute.For<IRepository<AwsS3ConfigGraphQLModel>>();
        _awsS3ConfigCache = new AwsS3ConfigCache(awsS3Repo, _eventAggregator);
        ...
        _validator = new S3StorageLocationValidator();
        _searchDebounce = new DebouncedAction(delayMs: TestDebounceDelayMs);

        _vm = new S3StorageLocationViewModel(
            _eventAggregator,
            _service,
    ...
    {
        PageType<S3StorageLocationGraphQLModel> emptyPage = new()
        {
            TotalEntries = 0,
            Entries = new ObservableCollection<S3StorageLocationGraphQLModel>()
        };
    ...
    {
        System.Action act = () => new S3StorageLocationViewModel(
            _eventAggregator, _service, _notificationService, _dialogService,
            _stringLengthCache, _awsS3ConfigCache, _joinableTaskFactory, _permissionCache,
            _validator, null!);
